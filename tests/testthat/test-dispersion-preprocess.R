...
test_that("the dispersion MLE reproduces optimize() over dnbinom()", {
  ...
  expect_identical(PACE:::pace_dispersion_mle_cpp(y, mu, FALSE, FALSE, Inf),
                   exp(stats::optimize(nb1, interval = c(-6, 4))$minimum))
  expect_identical(PACE:::pace_dispersion_mle_cpp(y, mu, TRUE, FALSE, Inf),
                   exp(stats::optimize(nb2, interval = c(-6, 4))$minimum))
  # too few cells, and counts that are not whole numbers, have no MLE
  expect_true(is.na(PACE:::pace_dispersion_mle_cpp(y[1:5], mu[1:5], FALSE, FALSE, Inf)))
  # the zero-count collapse is the same algebra, so it lands within the
  # optimiser's own tolerance of the full sum
  collapsed <- PACE:::pace_dispersion_mle_cpp(y, mu, FALSE, TRUE, Inf)
  expect_equal(collapsed, PACE:::pace_dispersion_mle_cpp(y, mu, FALSE, FALSE, Inf),
               tolerance = 1e-4)
})
